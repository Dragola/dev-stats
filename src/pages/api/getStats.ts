...
export default async function getStats(req: NextApiRequest, res: NextApiResponse) {
   ...
   if (commitStats === -1) return res.status(500);
   ...
   if (!repoStats) return res.status(500);
   ...
   if (!languages) return res.status(500);
   ...
   if (!frameworks) return res.status(500);
   ...
   if (!userStats) return res.status(500);
